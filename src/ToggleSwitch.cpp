...
std::unique_ptr<TogglePainter> ToggleSwitchBody::PainterMaker(int16_t top, int16_t left, int16_t width, int16_t height)
{
...
void ToggleSwitchBody::Update(Subject *from)
{
...
bool ToggleSwitchBody::State() const
{
    return state_ == BUTTON::kToggleOn && position_counter_ == kPositionCounterMax;
}

bool ToggleSwitchBody::Check(bool is_high)
{
    state_ = state_context_.Check(is_high);
    return State();
}

void ToggleSwitchBody::ProceedNextPosition()
{
...
{
    // std::shared_ptr<ToggleSwitchBody> h = std::dynamic_pointer_cast<ToggleSwitchBody>(body());
    std::weak_ptr<ToggleSwitchBody> h = std::static_pointer_cast<ToggleSwitchBody>(body());
    return h.lock()->State();
}
