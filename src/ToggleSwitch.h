...
class ToggleSwitchBody : public ListenerBody
{
  ...
  private:
    ...
    ButtonStateContext state_context_; /* state manager */
  ...
  public:
    ToggleSwitchBody() = delete;
    ToggleSwitchBody(int16_t top, int16_t left, int16_t width, int16_t height);
...
class ToggleSwitch : public Listener
{
  public:
    ...
    {
        set_body(std::make_shared<ToggleSwitchBody>(top, left, width, height));
    }
    bool Check(bool is_high);
    bool State() const;
